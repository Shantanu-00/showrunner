...
Cloud Scheduler: `director-tick`      — every 2 min  → POST {api}/internal/tick (OIDC service account)
                 ...
                 `orphan-sweep` — hourly → POST {api}/internal/sweep. Four jobs in one pass:
                                  ...
                                  public-class TTL auto-wrap (spec 11 §1.3) · public-class
                                  cost-ceiling check, pause+flag-for-wrap on breach (spec 11 §1.4)
                                  [2026-08-31: "+ hard-purge of events wrapped since the previous
                                  sweep" retracted from this line — never built, see spec 11 §1.3]
```
...
- **Concurrent-live-event hard cap (spec 11 §1) — the platform's circuit breaker.** A public, unrestricted demo URL live for a month (rules-mandated, Judging Period ends Oct 1) is an unbounded cost surface: nothing stops a forwarded link or an adversarial tester from spinning up "live" events and running the full Gemini/Vision/Lyria/Veo pipeline against $150–$300 of credits with zero product-side governor. `MAX_CONCURRENT_LIVE_EVENTS=3` (env-configurable) is enforced transactionally at Go Live against `platform/liveEventCount` — but **only for `class=='public'` events**; the judge-mode event and the deployment owner's own dev events are a different class entirely and never compete for these slots (spec 11 §1.1). `public`-class events additionally auto-wrap after 60 min and auto-pause past a $3 cost ceiling — so even unlimited strangers trying this can't hold a slot indefinitely or run up real spend. This is a real production-readiness signal, not hackathon theater. (2026-08-31: "and hard-purge on the next sweep" retracted here — the wrap and the ceiling are built, the purge never was. See spec 11 §1.3.)
- **Judge mode (built S14, `/judge`):** the hosted URL lands on a guided page — "Judging Showrunner? Here's the 60-second loop." pre-populated `protected_demo` event (spec 11 §1.1 — exempt from the capacity cap, since it's the one thing that must always work), one-tap guest join with 3 sample photos bundled with the page (routed through the ordinary send sheet, so the consent choice is still made), and kiosk/gallery/repo links. **The Flight Deck is cut** (S12) and is no longer part of this page. **The autonomy beat is a live next-tick countdown, not a button:** the manual trigger is demoted below the fold and labelled a manual override, because a judge pressing "Run director now" seconds before reading *"without human intervention"* is a rules-§4 "must function as depicted" contradiction (EXECUTION-PLAN §7e row 11). The countdown reads `GET /v1/events/{id}/public`'s `director` block — a field on the API, never a security-rules exception (HANDOFF §4.22). **Testing credentials, including the host link, live in the Devpost submission-instructions field and are *not* printed on the page** — a host link is a bearer credential, and a stranger who found the page could otherwise freeze or wrap the judges' event. **A disclosure panel on the page names every difference from a real event** (spec 12 §1's "demo conveniences disclosed on-screen"), and claims only what is enforced. **The tour explicitly narrates the share toggle** — *"tap 'Share to the big screen' — everything is private-by-default"* — because consent defaults to the pool ring (spec 02 §4): without that step a judge's upload never reaches the kiosk and reads as breakage; with it, a potential failure mode becomes a consent-architecture demo moment. Judges get the wow-loop in under a minute with zero setup, and — because they're never on the public capacity-limited path — zero exposure to anything the cap/TTL/kill-switch above is doing to deter everyone else.
...
- **`autoPromoteEnrollees: bool`** (spec 11 §3.4) — when true, a fresh selfie-enrollment auto-tiers the enrollee to 1 (Inner Circle) so the judge-tour visitor sees `vipWeight` effects live. The enrollment handler honors this flag **only** when `event.class == 'protected_demo'`; it is a no-op on every other event regardless of the flag's value, so this can never become a way for a real guest to self-promote. **Ships `false` on the seeded judge event (S14):** by the rule above this is the one remaining branch a real host cannot trigger, its only purpose is to make a judge feel personally featured, and the seeded cast's tier-0/1 people demonstrate `vipWeight` just as well. The code and its gate stay; the flag is off.
- **`publicFloor: 0.0`** — set on the demo event through the **ordinary `event.publicFloor` field** (real events default 0.45, spec 04 §2), *not* through a `demoConfig` override. A judge's test photo (their desk, their badge) would fail the aesthetic floor, never turn `public`, and read as "it's broken." With the floor at 0, consent + Guardian alone decide `public`, so any consented safe upload hits the kiosk `just_in` strip < 5 s; quality still governs hero curation via the aesthetic score term. **Revised 2026-08-29 (S14):** `demoConfig.publicFloor` existed as a `protected_demo`-gated override and was **deleted** — it was a second path to a value the plain host-settable field already provided, and it was the only exposure logic in the system whose condition was whose event it is. The rule, now binding: *a demo convenience is honest if it is a configuration value a real host could also set; it is a thumb on the scale if it is a code branch keyed on whose event it is.* Set by `scripts/seed_judge_event.py`; disclosed **on the `/judge` page itself**, not only in the README.
