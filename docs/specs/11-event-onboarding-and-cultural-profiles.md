...
**`class` is not `access.mode`, and the collision in the word "public" is worth heading off explicitly** — the two live on the same event document and mean unrelated things:

| | `events/{eventId}.class` | `events/{eventId}.access.mode` (spec 08 §3) |
|---|---|---|
| Values | `protected_demo` · `internal_dev` · `public` | `open` · `invite` |
| Who sets it | **Server**, never client-settable | **Host**, any time, from the console |
| What it governs | *Platform* guardrails against the deployment's own cost exposure: the concurrency cap, the TTL, the spend ceiling, the purge policy | *Who may read this event*: whether becoming a member needs the host's code, and how many seats exist |

Every one of the four combinations is legal and meaningful. A `class: 'public'` event (a stranger's event, capacity-counted, 60-minute TTL, $3 ceiling) is very often `access.mode: 'invite'` — a real host running a real party who wants a code on the door — and flipping that mode changes nothing about its class, its TTL or its slot. Conversely the `protected_demo` event is `class: 'protected_demo'` and `access.mode: 'open'`, because a walkthrough that demands an invite code is not a walkthrough. Nothing in §1.2–§1.5 below reads `access.mode`, and nothing in spec 08 §3 reads `class`; keeping them uncoupled is why "the host locked their door" can never be mistaken for "the platform granted this event a reserved slot."
...
A capacity cap alone stops runaway *concurrent* spend but not a slot being squatted indefinitely — someone who goes live and never wraps holds their slot forever. Fix: any `public`-class event auto-wraps `PUBLIC_EVENT_MAX_LIVE_MINUTES` (default **60**) after `liveAt`, unconditionally — no activity check needed, because 60 minutes is already generous for a curious visitor and far more than a legitimate one-off poke needs. The hourly `orphan-sweep` (spec 09 §2) is extended to perform this check.

> **Retracted 2026-08-31 — the purge half of this section was never built, and it is deleted rather than left as an aspiration.** This paragraph used to continue: *"and, in the same pass, **hard-purge** (Firestore subtree + GCS objects, not the standard 30-day retention of spec 02 §5 …) any `public`-class event that has been `wrapped` since the previous sweep,"* bounding an abandoned event's lifecycle to two sweep cycles. `api/sweep.py::_sweep_guardrails` implements the TTL auto-wrap and the cost ceiling and **nothing else**: a grep of `backend/` for `purge`/`retention` returns only `retentionNoticeShown`, a consent-UI flag. A wrapped public event's media, faces and enrollments persist indefinitely. Spec 02 §5's 30-day retention is in the same position — asserted, never implemented — so the sentence's contrast was between two things that do not exist.
>
> Removed because a data-lifecycle promise with no enforcement is the one class of claim that is worse than silence: every other unbuilt row in this project is marked, and this one was load-bearing on a privacy story. **Still true and still built:** an abandoned public event stops costing money (60-minute auto-wrap, $3 ceiling → pause) and stops holding a capacity slot. What is *not* true is that it eventually disappears. The working deletion path is per-subject, not per-event: `DELETE /v1/events/{id}/people/me` (spec 02 §5) really does delete a person's face documents, embeddings, private profile, push registration and tombstone their uploads.
>
> If the purge is wanted later it belongs here as a fifth sweep case, and it needs to be written as carefully as it sounds: it is the only destructive, irreversible, unattended write in the system, and its blast radius is somebody's photographs.
...
**One thing this section has to be honest about since host approval became universal (spec 02 §3):** a first-time visitor's face-matched album is *held* until the host approves the claim, on every event including `protected_demo`. So "the app feels like it's about me" now arrives in two steps — their own uploads immediately, the face-matched rest on approval — and the pending state is a designed surface, not a gap (spec 12 §7). Tiering is unaffected either way: the person document is created at enrollment, so a tier promotion applies exactly as described below whether or not the claim has been approved yet.
...
- [ ] `class` and `access.mode` are provably independent: flipping a `class: 'public'` event to `access.mode: 'invite'` (and back) leaves its class, its held capacity slot, its TTL and its cost ceiling untouched; a `platformAdmin` changing a class leaves `access.mode`, `maxGuests` and `codeHash` untouched. Grep-level: no code path reads one to decide the other.
- [ ] A `public`-class event that never wraps auto-wraps at `PUBLIC_EVENT_MAX_LIVE_MINUTES` and frees its capacity slot. (The purge half of this criterion is retracted — see §1.3.)
